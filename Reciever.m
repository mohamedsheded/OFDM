...
function [ReceivedBits] = Reciever(ReceivedSignal,NumberBitsPerFrame,ModulationOrder)
       ...
       DetectedSymbols_branch1(ReceivedSignal_branch1>sqrt(ModulationOrder)-2)=sqrt(ModulationOrder)-1;
        DetectedSymbols_branch1(ReceivedSignal_branch1<=-sqrt(ModulationOrder)+2)=-sqrt(ModulationOrder)+1;
         ...
         ReceivedSymbolIndex_branch1=(DetectedSymbols_branch1+sqrt(ModulationOrder)+1)/2-1;  % Transform detected symbols into bits: decimal to binary
            DetectedBits_branch1=dec2bin(ReceivedSymbolIndex_branch1',log2(ModulationOrder)/2);   end
